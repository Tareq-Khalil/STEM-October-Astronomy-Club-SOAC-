 ...
 * ║  API key stored securely via /api/chat proxy          ║
 * ╚═══════════════════════════════════════════════════════╝
 */
(function () {
...
Mission: Explore the universe, foster scientific curiosity, inspire the next generation of astronomers.
...
- Open to: 9th grade and high school students worldwide. Team size: 3 students.
...
- Prizes: 1st Place VR 114-500 EQ Telescope; 2nd & 3rd get 3 AoPS coupons each; Wolfram prizes worth $111k+ (all participants 1-month Wolfram|One, top 16 get 1-year licenses, special $500 Wolfram Summer Program scholarship); Gold/Silver/Bronze medals + certificates for all
- Sponsors: Wolfram, Art of Problem Solving (AoPS)

== SIMULATIONS (11 total) ==
Curved Spacetime, Orbital Mechanics, Black Hole Visualization, Solar System 3D, Circumstellar Habitable Zone, Eclipsing Binary, Artificial Satellites, and more.

== GAMES ==
Space Odyssey, Astro Hunt, Planet Explorer, Space Mission Simulator, Asteroid Dodge.

== WEBSITE ==
soac.club — Home, About Us, Simulations, Events, Competitions, Games, Partners, Contact.

If asked something not covered above, say so and suggest contacting the club directly.`;
  ...
  const CSS = `
    #cosmos-btn {
      ...
      border:2px solid rgba(135,206,250,.65);cursor:pointer;z-index:2147483646;
      display:flex;align-items:center;justify-content:center;font-size:25px;
      box-shadow:0 0 22px rgba(100,180,255,.45),0 4px 16px rgba(0,0,0,.55);
      transition:transform .25s,box-shadow .25s;animation:cmos-pulse 3s ease-in-out infinite;
    }
    #cosmos-btn:hover{transform:scale(1.11);box-shadow:0 0 34px rgba(100,180,255,.75),0 4px 20px rgba(0,0,0,.55);}
    ...
    #cosmos-panel{
      position:fixed;bottom:98px;right:28px;width:368px;height:528px;
      background:linear-gradient(160deg,#080c24 0%,#0c1836 55%,#080c24 100%);
      border:1px solid rgba(135,206,250,.28);border-radius:22px;z-index:2147483647;
    ...
    .cmos-header{
      padding:15px 18px;background:linear-gradient(135deg,rgba(138,43,226,.28),rgba(72,187,255,.18));
      border-bottom:1px solid rgba(135,206,250,.18);display:flex;align-items:center;
      gap:11px;position:relative;z-index:1;flex-shrink:0;
    }
    .cmos-avatar{
      width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#6a11cb,#2575fc);
      border:2px solid rgba(135,206,250,.5);display:flex;align-items:center;justify-content:center;
    ...
    .cmos-x{background:none;border:none;color:rgba(135,206,250,.55);font-size:21px;cursor:pointer;
      padding:1px 5px;border-radius:6px;line-height:1;transition:color .18s,background .18s;}
    .cmos-x:hover{color:#87ceeb;background:rgba(135,206,250,.1);}
    .cmos-msgs{
      flex:1;overflow-y:auto;padding:14px 14px 6px;display:flex;flex-direction:column;gap:11px;
      position:relative;z-index:1;scrollbar-width:thin;scrollbar-color:rgba(135,206,250,.28) transparent;
    }
    .cmos-msgs::-webkit-scrollbar{width:3px;}
    .cmos-msgs::-webkit-scrollbar-thumb{background:rgba(135,206,250,.28);border-radius:2px;}
    .cmos-bubble{
      max-width:90%;padding:9px 13px;border-radius:14px;font-size:13.2px;
      line-height:1.58;white-space:pre-wrap;word-break:break-word;animation:cmos-in .28s ease;
    }
    ...
    .cmos-td{width:7px;height:7px;border-radius:50%;background:rgba(135,206,250,.7);animation:cmos-td 1.3s ease-in-out infinite;}
    ...
    .cmos-foot{
      padding:10px 12px;border-top:1px solid rgba(135,206,250,.13);display:flex;gap:7px;
      align-items:flex-end;position:relative;z-index:1;background:rgba(8,12,36,.65);flex-shrink:0;
    }
    .cmos-inp{
      flex:1;background:rgba(18,28,62,.82);border:1px solid rgba(135,206,250,.22);
      border-radius:11px;color:#cfe6ff;font-size:13px;padding:8px 12px;resize:none;
      font-family:inherit;outline:none;max-height:96px;min-height:36px;line-height:1.45;transition:border-color .18s;
    }
    .cmos-inp::placeholder{color:rgba(135,206,250,.38);}
    .cmos-inp:focus{border-color:rgba(135,206,250,.52);}
    .cmos-send{
      width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,#6a11cb,#2575fc);
      border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;
      flex-shrink:0;transition:transform .18s,box-shadow .18s;box-shadow:0 2px 10px rgba(100,150,255,.3);
    }
    ...
    .cmos-chips{display:flex;flex-wrap:wrap;gap:6px;padding:0 14px 10px;position:relative;z-index:1;}
    .cmos-chip{
      background:rgba(100,60,180,.35);border:1px solid rgba(135,206,250,.25);
      color:rgba(200,225,255,.82);font-size:11.5px;padding:5px 10px;border-radius:20px;
      cursor:pointer;transition:background .18s,border-color .18s;font-family:inherit;white-space:nowrap;
    }
  ...
  async function ask(text) {
    ...
    try {
      ...
      // Calls your own /api/chat Vercel serverless function — key stays on the server
      const res = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: OPENROUTER_MODEL, messages, max_tokens: 600, temperature: 0.7 }),
      });
